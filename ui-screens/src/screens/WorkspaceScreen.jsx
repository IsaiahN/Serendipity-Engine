...
/* ─── Comparison Export Helper ─── */
function buildComparisonMarkdown(data, dimensions) {
  const lines = [];
  lines.push(`# Deep Comparison: ${data.workA.title} vs ${data.workB.title}`);
  lines.push('');
  lines.push(`**Overall Divergence:** ${data.overallDivergence}%`);
  lines.push('');
  if (data.oneSentenceSummary) {
    lines.push(`> ${data.oneSentenceSummary}`);
    lines.push('');
  }
  if (data.topChanges && data.topChanges.length > 0) {
    lines.push('## Top Changes');
    lines.push('');
    for (const change of data.topChanges) {
      lines.push(`- ${change}`);
    }
    lines.push('');
  }
  ...
  for (const d of dimensions) {
    ...
    if (!dim) continue;
    lines.push(`## ${d.label} (${dim.divergence}% divergent)`);
    lines.push('');
    if (dim.keyDiff) {
      lines.push(`**Key Difference:** ${dim.keyDiff}`);
      lines.push('');
    }
    lines.push(`### ${data.workA.title}`);
    lines.push('');
    lines.push(`${dim.summaryA || 'No analysis available.'}`);
    if (dim.scoreA != null) lines.push(`\n**Score:** ${dim.scoreA}/10`);
    lines.push('');
    lines.push(`### ${data.workB.title}`);
    lines.push('');
    lines.push(`${dim.summaryB || 'No analysis available.'}`);
    if (dim.scoreB != null) lines.push(`\n**Score:** ${dim.scoreB}/10`);
    lines.push('');
    lines.push('---');
    lines.push('');
  }
  lines.push(`*Generated by Serendipity | StoryWeaver*`);
  return lines.join('\n');
}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: showRadar ? '1fr 320px' : '1fr', gap: 16 }}>
        ...
        {showRadar && (
          <div style={{ position: 'sticky', top: 0 }}>
            ...
            {/* Export */}
            <Card style={{ padding: 16, marginTop: 10 }}>
              <Button variant="secondary" style={{ width: '100%', marginBottom: 6 }} onClick={async () => {
                if (!data) return;
                const md = buildComparisonMarkdown(data, comparisonDimensions);
                const { downloadFile } = await import('../services/exportEngine.js');
                const safeName = `${data.workA.title} vs ${data.workB.title}`.replace(/[^a-zA-Z0-9 _-]/g, '').trim();
                downloadFile(md, `${safeName} - Deep Comparison.md`, 'text/markdown');
              }}>
                <Download size={12} style={{ marginRight: 4 }} /> Export as Markdown
              </Button>
              <Button variant="secondary" style={{ width: '100%' }} onClick={async () => {
                if (!data) return;
                const md = buildComparisonMarkdown(data, comparisonDimensions);
                const { downloadFile } = await import('../services/exportEngine.js');
                const safeName = `${data.workA.title} vs ${data.workB.title}`.replace(/[^a-zA-Z0-9 _-]/g, '').trim();
                // Build a minimal raw PDF from the markdown text
                ...
                const pageW = 612;
                const pageH = 792;
                const maxLineW = pageW - marginX * 2;
                const charW = fontSize * 0.52; // approximate monospace width
                ...
                for (const line of wrapped) {
                  ...
                  const fs = isHeader ? 13 : fontSize;
                  const ld = isHeader ? 18 : leading;
                  y -= ld;
                  if (y < 50) { pages.push(pageLines); pageLines = []; y = pageH - marginTop - ld; }
                  pageLines.push({ text: line.replace(/^#+\s*/, ''), fs, y, bold: isHeader });
                }
                ...
                for (const pg of pages) {
                  let stream = '';
                  for (const ln of pg) {
                    stream += `BT\n/F1 ${ln.fs} Tf\n${marginX} ${ln.y.toFixed(1)} Td\n(${escPdf(ln.text)}) Tj\nET\n`;
                  }
                  const streamId = addObj(`<< /Length ${stream.length} >>\nstream\n${stream}endstream`);
                  const pgId = addObj(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${pageW} ${pageH}] /Contents ${streamId} 0 R /Resources << /Font << /F1 ${fontId} 0 R >> >> >>`);
                  pageIds.push(pgId);
                }
                objs[pagesId - 1].content = `<< /Type /Pages /Kids [${pageIds.map(id => `${id} 0 R`).join(' ')}] /Count ${pageIds.length} >>`;
                ...
                downloadFile(blob, `${safeName} - Deep Comparison.pdf`, 'application/pdf');
              }}>
